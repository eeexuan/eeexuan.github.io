...
function searchVideo(){
  let searchTxt = document.getElementById("txt-search").value
  console.log(`you searched for videos relating to ${searchTxt}!`)
  //clear the textbox
  document.getElementById("txt-search").value = ""
  //error if search box is empty
  if (searchTxt === "") {
  ...
  for (let i = 0; i < videosList.length; i++) {
    let related = videosList[i].description.toUpperCase().includes(searchTxt.toUpperCase())
    if (related === true) {
      let video = document.createElement('div')
      video.innerHTML = generateRowHTML(videosList[i])
      document.getElementById("results").appendChild(video)
      numVideos++
    } 
  }
  if (numVideos === 0) {
    document.getElementById("results").innerHTML = `No videos found with the keyword ${searchTxt}`
  }
}

function showAll(){
  console.log("you clicked on the show all button!")
  //clear the previous videolist
  document.getElementById("results").innerHTML = ""
  //display all videos
  for (let i = 0; i < videosList.length; i++) {
      let video = document.createElement('div')
